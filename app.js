const CONFIG = {
  ...
  demoPassword: "Ab@12",
  rooms: 9,
  saveWebhookUrl: "https://script.google.com/macros/s/AKfycbw0FcFm2X1_sQAlm9X2MLeOqJuIcdz0Kq6XVfxW_fQ1rbOmQx7dvG8YCTTKpPU80PCszQ/exec",
  resetWebhookUrl: "",
  admins: {
...
function navigate(section) {
  const isAdmin = Boolean(authenticatedAdmin || sessionStorage.getItem("roomflow_admin"));
  if (section === "roomPrice" && !isAdmin) {
    showPopup("error", "Access Denied", "Only Admin users can access Room Price settings.");
    return;
  }
...
/* ============================================================
   COUNTDOWN & LIVE OVERTIME COUNTER WITH EXPIRATION POPUP
   ============================================================ */
...
function updateCountdowns() {
  document.querySelectorAll(".countdown-display").forEach(element => {
    ...
    // TRIGGER POPUP ONCE WHEN COUNTDOWN EXPIRES
...
/* Dashboard Render with Free Birthday Hall Support */
function render() {
  if (!$("roomGrid")) return;

  clients = deduplicateClients(clients);

  // Map occupied rooms and free hall reservations
  const occupied = new Map();

  clients.filter(client => client.status === "Occupied").forEach(client => {
    // 1. Mark main allotted room
    occupied.set(String(client.room), client);

    // 2. If client has Complimentary Free Hall, also mark "Hall" as occupied/reserved
...
/* ============================================================
   100% DYNAMIC ROOM PRICE MANAGEMENT (NO HARDCODED VALUES)
   ============================================================ */
...
function bindAllEvents() {
  ...
  if ($("clientForm")) {
    $("clientForm").onsubmit = async event => {
      ...
      // 1. Check if selected main room is already occupied
      ...
      // 2. Check if Complimentary Free Hall is selected but Hall is already occupied or reserved
...
function toggleFreeHallCheckbox() {
  ...
  // 1. HIDE checkbox if no room selected OR if "Hall For Celebration" is selected directly
  ...
  // 2. SHOW checkbox for regular room selection (101, 102, 201, etc.)
  container.classList.remove("hidden");

  // 3. Check if Hall is ALREADY occupied or reserved by any active client
...
// Attach event listener inside bindAllEvents()
if ($("room")) {
